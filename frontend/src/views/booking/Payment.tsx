...
export default function Payment() {
  ...
  const handlePay = async () => {
    ...
    const res = await submitBooking(total, method);
    if (res.success) {
      if ((method === 'atm' || method === 'sepay') && res.paymentUrl && res.paymentId && res.expiredAt) {
        setSepayData({
          url: res.paymentUrl,
          id: res.paymentId,
  ...
  return (
    <>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-700">
            ...
            <div className="space-y-4">
              ...
              <label className={`flex items-center gap-4 p-4 border-2 rounded-xl cursor-pointer ${method === 'atm' ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700'}`}>
                ...
                <span className="font-medium">Chuyển khoản / Quét mã QR (SePay)</span>
              </label>
            </div>
          ...
          <div className="flex justify-between">
            ...
            <button 
              onClick={handlePay}
              disabled={isLoading}
              className="px-8 py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-xl font-bold"
            >
      ...
      {sepayData && (
        <SePayModal 
          paymentUrl={sepayData.url} 
          paymentId={sepayData.id} 
          expiredAt={sepayData.expiredAt} 
