...
export default function AddOns() {
  ...
  const [options, setOptions] = useState<{ id: string; title: string; desc: string; price: number; icon: string }[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      try {
        ...
        const res = await api.get('/flights/ancillary/options');
        if (res.data?.data?.addons || res.data?.addons) {
          setOptions(res.data?.data?.addons || res.data?.addons);
        }
      } catch (e) {
        // Fallback
        setOptions([
  ...
  const renderIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'car': return <Car className="w-8 h-8 text-blue-500" />;
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-700">
      ...
      {loading ? (
        <div className="flex justify-center py-8">
          <span className="text-blue-500 animate-pulse">Đang tải các dịch vụ...</span>
        </div>
      ) : (
        <div className="space-y-4 mb-8">
          {options.map(opt => (
            ...
            >
              <div className="shrink-0">{renderIcon(opt.icon)}</div>
