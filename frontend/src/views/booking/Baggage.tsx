...
export default function Baggage() {
  ...
  const [options, setOptions] = useState<{ id: string; weight: number; price: number; label: string }[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      try {
        ...
        if (res.data?.data?.baggage || res.data?.baggage) {
          setOptions(res.data?.data?.baggage || res.data?.baggage);
        }
      } catch (e) {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-700">
      ...
      {loading ? (
        <div className="flex justify-center py-8">
          <span className="text-blue-500 animate-pulse">Đang tải các gói hành lý...</span>
        </div>
      ) : (
      ...
      <div className="flex justify-between">
        <button 
          onClick={() => navigate.push('/booking/seat')}
